#!/usr/bin/env dotnet
// Task runner for the Maple.StringPool repository.
...
switch (cmd)
{
    case "bench":
        Run(
            "dotnet",
            "run --project src/Maple.StringPool.Benchmarks/Maple.StringPool.Benchmarks.csproj"
    ...
    case "comparison-bench":
        Run(
            "dotnet",
            "run --project src/Maple.StringPool.ComparisonBenchmarks/Maple.StringPool.ComparisonBenchmarks.csproj"
                + " -c Release -- run",
            repoRoot
        );
        break;
    ...
    case "disasm":
        Run(
            "dotnet",
            "run --project src/Maple.StringPool.Benchmarks/Maple.StringPool.Benchmarks.csproj"
                + " -c Release -- --filter \"*StringPool*\" --disasm",
            repoRoot
        );
        break;
    ...
    case "publish-cli":
        ...
        Run(
            "dotnet",
            $"publish src/Maple.StringPool.Cli/Maple.StringPool.Cli.csproj -c Release -r {rid} --self-contained false",
            repoRoot
        );
        break;
    ...
    default:
        Console.WriteLine(
            ...
            Commands:
              ...
              publish-cli [RID]         Publish sp.exe CLI (defaults to current platform RID)
